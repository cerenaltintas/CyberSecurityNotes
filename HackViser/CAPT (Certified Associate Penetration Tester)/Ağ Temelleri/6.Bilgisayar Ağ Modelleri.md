...
1. **Fiziksel Katman (Physical Layer):** Donanım düzeyindeki iletişimi, kablolar, kartlar ve diğer fiziksel yapılar üzerinden veri aktarımını sağlar.
2. **Veri Bağlantı Katmanı (Data Link Layer):** Ağ üzerindeki veri akışını düzenleyerek, verilerin doğru adreslere ulaşmasını sağlar. MAC (Media Access Control) adreslerini kullanır.
3. **Ağ Katmanı (Network Layer):** Verilerin farklı ağlar arası iletimini yönetir. Bu katmanda IP adresleri ve yönlendirme işlemleri bulunur.
4. **Taşıma Katmanı (Transport Layer):** Uçtan uca iletişimi yönetir ve veri aktarımının güvenilirliğini sağlar. TCP (Transmission Control Protocol) ve UDP (User Datagram Protocol) bu katmanda yer alır.
5. **Oturum Katmanı (Session Layer):** İletişim oturumlarını yönetir, bağlantıların başlatılmasını ve sonlandırılmasını kontrol eder.
6. **Sunum Katmanı (Presentation Layer):** Verilerin ağ üzerinden nasıl temsil edileceğini yönetir. Şifreleme ve sıkıştırma işlemleri bu katmanda gerçekleşir.
7. **Uygulama Katmanı (Application Layer):** Kullanıcıların ağ üzerindeki uygulamalarla etkileşime geçebilmesini sağlar. E-posta, web tarayıcıları ve diğer uç kullanıcı uygulamaları bu katmanda yer alır.
