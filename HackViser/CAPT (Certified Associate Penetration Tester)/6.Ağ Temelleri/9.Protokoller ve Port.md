...
```
LDAPTLS_REQCERT=never ldapsearch -x -H ldaps://192.168.1.50:636 -b "dc=sirket,dc=local"
```
...
```
ssh kullanıcı_adı@192.168.1.50
```
- Belirli bir port (örn: 2222) kullanılıyorsa:
```
  ssh kullanıcı_adı@192.168.1.50 -p 2222
```
...
- **Komut Örneği (Sunucuya bağlanıp selam verme):**
```
telnet 192.168.1.50 25
Bağlandıktan sonra şunu yazarsın:
HELO sirket.local
