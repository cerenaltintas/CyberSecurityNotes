Feroxbuster, web uygulamalarında gizli kalan veya unutulmuş dizinleri ve dosyaları keşfetmek için tasarlanmış, komut satırı tabanlı bir araçtır. Rust programlama dilinde yazılmış olan Feroxbuster, yüksek performansı ve etkili tarama kabiliyeti ile dikkat çeker. Güvenlik profesyonelleri, sızma testçileri ve web geliştiriciler tarafından sıkça kullanılan bu araç, kaba kuvvet (brute-force) ve sözlük saldırıları (wordlist attacks) yöntemlerini kullanarak web sunucularını tarar.

### Özellikler

**Yüksek Performans:** Rust programlama dilinde yazılmış olması, onu oldukça hızlı ve verimli kılar.

**Esneklik:** Tarama işlemleri sırasında çeşitli parametreleri özelleştirebilir, örneğin istek başına iş parçacığı sayısı, tarama derinliği ve zaman aşımı sürelerini ayarlayabilirsiniz.

**Çok Yönlülük:** Feroxbuster, sadece dizin ve dosya taraması yapmakla kalmaz, aynı zamanda farklı tarama modlarına da sahiptir. Bu modlar arasında, yinelemeli tarama ve vurgulama (highlighting) gibi özellikler bulunur.

Basit bir kullanım senaryosunda, Feroxbuster'ı belirli bir web sitesindeki dizinleri ve dosyaları taramak için kullanabilirsiniz. Aşağıdaki komut, temel bir tarama işlemi gerçekleştirir:
...
Aşağıdaki komutla birlikte hedef sistem üzerinde bir dosya taraması yapalım. Dosya/sayfa taraması yaparken
...
parametresiyle uzantı belirtiriz.
